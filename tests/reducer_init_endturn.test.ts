...
describe('reducer INIT and END_TURN', () => {
  test('INIT generates world and updates seed and map', () => {
    ...
    const next = applyAction(s, { type: 'INIT', payload: { seed: 'abc', width: 3, height: 3 } } as any);
  ...
  test('END_TURN advances research and emits unlock when complete', () => {
    ...
    s.techCatalog = [{ id: 't1', tree: 'science', name: 'T1', cost: 1, prerequisites: [], effects: [] } as any];
    s.players = [{ id: 'p1', isHuman: true, leader: { id: 'l', name: 'L', aggression: 0.5, scienceFocus: 0.5, cultureFocus: 0.5, expansionism: 0.5 }, sciencePoints: 1, culturePoints: 0, researchedTechIds: [], researching: { techId: 't1', progress: 0 } } as any];
