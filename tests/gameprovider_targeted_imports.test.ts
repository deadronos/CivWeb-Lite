test('simulateAdvanceTurn calls globalGameBus.emit and dispatches END_TURN', async () => {
  ...
  vi.doMock('../src/game/ai/ai', () => ({ evaluateAI: (p: any, s: any) => [{ type: 'LOG', payload: 'ai-act' }] }));

  const gp = await import('../src/contexts/GameProvider');
  const s = gp.initialStateForTests();
  // add one AI player
  s.players = [{ id: 'ai1', isHuman: false, leader: { id: 'L1', name: 'AI' }, researchedTechIds: [], researching: null, sciencePoints: 0, culturePoints: 0 }];
  const actions: any[] = [];
  const dispatch = (a: any) => actions.push(a);
  gp.simulateAdvanceTurn(s, dispatch);
