...
type StartConfig = { width: number; height: number; seed?: string; totalPlayers: number; humanPlayers: number; selectedLeaders?: Array<string|'random'|undefined> };
...
export default function MainMenu({ onStart }: { onStart(config: StartConfig): void }) {
  ...
  const [selectedLeaders, setSelectedLeaders] = React.useState<Array<string|'random'|undefined>>(['random','random','random','random','random','random']);
    ...
    <div role="dialog" aria-modal="true" aria-label="Main Menu" style={styles.backdrop}>
      <form onSubmit={start} style={styles.panel}>
        ...
        <div style={styles.field}>
          ...
          <input id="seed" value={seed} onChange={e => setSeed(e.target.value)} placeholder="random if blank" />
        </div>
        <div style={styles.field}>
          <label htmlFor="players">Players (total)</label>
          <input id="players" type="number" min={1} max={6} value={players} onChange={e => setPlayers(parseInt(e.target.value || '2', 10))} />
        </div>
            ...
            <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'space-between', marginTop: 6 }}>
              ...
              >
                ...
                {(leaders as any[]).map(l => (
                  <option key={l.id} value={l.id} title={`${l.historical_note || ''} | Victory: ${(l.preferred_victory||[]).join(', ')}`}>{l.name}</option>
                ))}
              </select>
            </div>
          ))}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          ...
          <button type="button" onClick={() => document.getElementById('hud-load-input')?.click()}>Load Save…</button>
          <input id="load-input" type="file" accept="application/json" style={{ display: 'none' }} onChange={onLoadFile} />
        </div>
        <p style={{ fontSize: 12, opacity: 0.8, marginTop: 12 }}>Tip: You can paste JSON in the in‑game load panel.</p>
      </form>
    </div>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  panel: {
    background: 'var(--color-bg, #1e1e1e)', color: 'var(--color-fg, #f0f0f0)', padding: 20, borderRadius: 8, width: 360, boxShadow: '0 8px 24px rgba(0,0,0,0.4)'
  },
  field: { display: 'flex', gap: 8, alignItems: 'center', justifyContent: 'space-between', marginTop: 8 },
};
