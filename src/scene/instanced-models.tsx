...
export default function InstancedModels({ geometry, material, positions, size = 0.5, elevations, onPointerMove }: InstancedModelsProps) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const e = elevations && elevations[i] != null ? elevations[i] : 0.5;
      ...
      object.updateMatrix();
      (mesh as any).setMatrixAt(i, object.matrix);
    }
    if ((mesh as any).instanceMatrix) (mesh as any).instanceMatrix.needsUpdate = true;
  }, [positions, elevations, size, object, count]);

  return (
    <instancedMesh ref={ref} args={[geometry as any, material as any, count]} onPointerMove={onPointerMove as any} />
  );
}
